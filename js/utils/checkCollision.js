...
}               //POPRAWIC KOLIZJE Z ENEMY I BUFF

const checkPlayerEnemyCollision = (player, enemies) => {
    enemies.forEach(enemy => {
        if (((player.position.x < enemy.position.x && player.position.x + player.width > enemy.position.x) || (player.position.x > enemy.position.x && player.position.x < enemy.position.x + enemy.width)) && player.position.y < enemy.position.y + enemy.height) {
            enemies.splice(enemies.indexOf(enemy), 1)
            player.takeHit()
        }
    })
...
const checkPlayerBuffCollision = (player, buffs) => {
    buffs.forEach(buff => {
        if (((player.position.x < buff.position.x && player.position.x + player.width > buff.position.x) || (player.position.x > buff.position.x && player.position.x < buff.position.x + buff.width)) && player.position.y < buff.position.y + buff.height) {
            buffs.splice(buffs.indexOf(buff), 1)
